...
def addPastRain(pdInput,rain):
    ...
    for i in range(len(pdInput.index)):
        ...
        for j in range(1,23):
            ...
            averageRain.append(totalPastRain[j]/(j+1))
            pdInput.at[i,'rainAverage'+str(j)] = averageRain[j-1]
        print(pdInput)
    return pdInput

def movingAverage(lInput):
    if len(lInput) == 0:
        return
    elif len(lInput) == 1:
        return lInput 
    ...
    lOutput = []
    total = lInput[0]+lInput[1]
    lOutput.append(total/2)

    if len(lInput) == 2:
        return lOutput

    for i in range(2,len(lInput)):
        total += lInput[i]
        lOutput.append(total/(i+1))
    return lOutput
...
def addHourlyRain(pdInput, rain, nrHours):
    ...
    for i in range(len(pdInput.index)):
        ...
        if nrHours > pdInput.iloc[i]['hour']:
            ...
            totalPastRain = np.concatenate((npPrevday,npDay))
            print(movingAverage(totalPastRain))
...
def rainAttributes2(pdInput, rain, saveFile):
...
if __name__ == '__main__':
    ...
    print("4/4")
    print(rain_2)
    startDate = pd.Timestamp(2017,2,1)
    endDate = pd.Timestamp(2021,3,1)
        
    print("set proper date format")
    pd112.loc[:,'date'] = pd.to_datetime(pd112['date'], format='%Y-%m-%d')
    print("filter dates")
    pd112 = pd112[(pd112['date']>startDate)&(pd112['date']<endDate)]
    pd112 = pd112.reset_index(drop=True)
    print(pd112)
    # #output = combineDataFrames(pd112 = pd112, pdRain=rain, saveFile=folder+'112RainSample2.csv')
    output = rainAttributes2(pd112, rain_2, folder+"112RainSumSample.csv")
    # output = dayRain(pd112_2, rain, folder+'112DayRainSample.csv')

    # print(output)

    inputlist = [1,2,34,42,345]
    print(movingAverage(inputlist))